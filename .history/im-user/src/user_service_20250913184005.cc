							...
							google::protobuf::Closure *done)
{
	...
	if (!db_->update(update_sql)) {
		std::cout << "Failed to update user state to online for uid=" << uid << std::endl;
	}
							...
							google::protobuf::Closure *done)
{
	...
	if (!db_->update(update_sql)) {
		std::cout << "Failed to update user state to offline for uid=" << req->user_id() << std::endl;
	}
...
void UserServiceImpl::resetAllUsersToOffline()
{
	...
	if (db_->update(sql)) {
		std::cout << "All users reset to offline state on service startup" << std::endl;
	} else {
		std::cout << "Failed to reset users to offline state" << std::endl;
	}
}