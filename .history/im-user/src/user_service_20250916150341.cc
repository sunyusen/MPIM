...
UserServiceImpl::UserServiceImpl()
{
	...
	// 初始化Redis缓存
	user_cache_.reset(new mpim::cache::BusinessCacheImpl("user", mpim::cache::CacheConfig{}));
							...
							google::protobuf::Closure *done)
{
	...
	// 1. 先查Redis缓存
	std::string cache_key = getUserCacheKey(u);
	std::string cached_data = user_cache_->Get(cache_key);
	...
	user_cache_->Set(cache_key, user_data, 3600); // 缓存1小时
