...
UserServiceImpl::UserServiceImpl()
{
    ...
    // 初始化连接池（可根据机器核数或配置调整），支持环境变量 MPIM_DB_POOL_SIZE 覆盖
    MySQLPool::instance().configure(8);
							...
							google::protobuf::Closure *done)
{
	...
	if (!cached_data.empty()) {
		...
		if (cached_user_id > 0) {
            ...
            auto db = MySQLPool::instance().acquire_for(std::chrono::milliseconds(200));
            MYSQL_RES *res = nullptr;
            if (!db) {
                LOG_WARN << "UserServiceImpl::Login: acquire DB timeout during cache-verify, skip cache fast-path";
            } else {
                res = db->query(sql);
            }
    ...
    auto db = MySQLPool::instance().acquire_for(std::chrono::milliseconds(500));
    auto *res = db ? db->query(sql) : nullptr;
	if (!res)
	{
    ...
    auto db2 = MySQLPool::instance().acquire_for(std::chrono::milliseconds(200));
    MYSQL_RES *check_res = db2 ? db2->query(check_online_sql) : nullptr;
    if (check_res) {
        MYSQL_ROW check_row = mysql_fetch_row(check_res);
        if (check_row && strcmp(check_row[0], "online") == 0) {
            // 用户已在线：允许本次登录继续，避免并发压测残留状态导致拒绝
            LOG_WARN << "UserServiceImpl::Login: user " << uid << " already online, allow relogin";
        }
    ...
    {
        auto db3 = MySQLPool::instance().acquire_for(std::chrono::milliseconds(200));
        if (!db3) {
            LOG_WARN << "UserServiceImpl::Login: acquire DB timeout for set-online, skip";
        } else if (!db3->update(update_sql)) {
...
bool UserServiceImpl::userExists(const std::string& username, long long* id_out)
{
    ...
    auto db = MySQLPool::instance().acquire();
