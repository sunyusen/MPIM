							...
							google::protobuf::Closure *done)
{
	...
	if (!db_->update(update_sql)) {
		LOG_ERROR("Failed to update user state to online for uid=" + std::to_string(uid));
	}
							...
							google::protobuf::Closure *done)
{
	...
	if (!db_->update(update_sql)) {
		LOG_ERROR("Failed to update user state to offline for uid=" + std::to_string(req->user_id()));
	}
