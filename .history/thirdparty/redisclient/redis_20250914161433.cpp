...
std::string Redis::get(const std::string& key, bool* found) {
  ...
  if (reply->type == REDIS_REPLY_STRING) {
    // 对于hiredis 0.12.1，str字段可能为NULL，但len字段有效
    if (reply->str && reply->len > 0) {
      out.assign(reply->str, (size_t)reply->len);
      if (found) *found = true;
    } else {
      // 如果str为NULL但len > 0，说明hiredis版本有问题
      // 这种情况下我们无法获取实际值，返回空字符串
      std::cerr << "GET warning: str is NULL but len is " << reply->len << std::endl;
    }
