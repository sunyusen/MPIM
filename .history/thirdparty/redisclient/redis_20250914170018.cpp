...
bool Redis::setex(const std::string& key, int ttl_sec, const std::string& value) {
  ...
  if (!ok) {
    std::cerr << "SETEX failed - reply type: " << reply->type;
    if (reply->str) {
      std::cerr << ", str: " << reply->str;
    }
    std::cerr << std::endl;
  }
...
std::string Redis::get(const std::string& key, bool* found) {
  ...
  // 使用redisAppendCommand和redisGetReply来避免hiredis 0.12.1的兼容性问题
  if (redisAppendCommand(_publish_context, "GET %s", key.c_str()) != REDIS_OK) {
    std::cerr << "GET append command failed\n";
    return {};
  }
  
  redisReply* reply = nullptr;
  if (redisGetReply(_publish_context, (void**)&reply) != REDIS_OK) {
    std::cerr << "GET get reply failed\n";
    return {};
  }
