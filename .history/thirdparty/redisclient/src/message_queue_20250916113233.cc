...
bool MessageQueue::Connect() {
    ...
    if (publish_context_ == nullptr || publish_context_->err) {
        if (publish_context_) {
            std::cerr << "MessageQueue publish connection error: " << publish_context_->errstr << std::endl;
    ...
    if (subscribe_context_ == nullptr || subscribe_context_->err) {
        if (subscribe_context_) {
            std::cerr << "MessageQueue subscribe connection error: " << subscribe_context_->errstr << std::endl;
    ...
    std::cout << "MessageQueue connected successfully" << std::endl;
    return true;
}
