...
int main(int argc, char** argv) {
    ...
    int from = 1, to = 100; // [from, to]

    for (int i=1;i<argc;i++){
        std::string a=argv[i];
        auto eat=[&](const char* k, auto& dst){ if(a.rfind(k,0)==0){ dst=decltype(dst)(std::stoi(a.substr(strlen(k)))); return true;} return false;};
        if (a.rfind("--host=",0)==0) host = a.substr(7);
        else if (a.rfind("--port=",0)==0) eat("--port=", port);
        else if (a.rfind("--base=",0)==0) base = a.substr(7);
        else if (a.rfind("--password=",0)==0) password = a.substr(11);
        else if (a.rfind("--from=",0)==0) eat("--from=", from);
        else if (a.rfind("--to=",0)==0) eat("--to=", to);
    }

    int fd = ::socket(AF_INET, SOCK_STREAM, 0);
    if (fd == -1) { std::cerr<<"socket fail\n"; return 1; }
    int one=1; setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &one, sizeof(one));
    sockaddr_in addr{}; addr.sin_family=AF_INET; addr.sin_port=htons(port); addr.sin_addr.s_addr=inet_addr(host.c_str());
    if (-1 == ::connect(fd,(sockaddr*)&addr,sizeof(addr))) { std::cerr<<"connect fail\n"; return 1; }
    std::string wl; recv_line(fd, wl);

    for (int i=from;i<=to;i++){
        std::string u = base + std::to_string(i);
        if (!send_line(fd, std::string("REGISTER ")+u+" "+password)) { std::cerr<<"send fail\n"; break; }
        std::string resp; if(!recv_line(fd, resp)) { std::cerr<<"recv fail\n"; break; }
        std::cout << u << ": " << resp << "\n";
    }
    ::close(fd);
    return 0;
