...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < cmd.pairs; i++)
	{
        ...
        if (!send_line(a, "LOGIN " + userA + " " + cmd.password))
			return 1;
        std::string ln;
        if (!recv_line_timeout(a, ln, 5000) || ln.rfind("+OK", 0) != 0)
		{
			std::cerr << "login A fail: " << ln << "\n";
			return 1;
		}
        else {
            size_t p = ln.find("uid=");
            if (p != std::string::npos) {
                try { cps[i].uidA = std::stoll(ln.substr(p+4)); } catch (...) { cps[i].uidA = 0; }
            }
        }
		if (!send_line(b, "LOGIN " + userB + " " + cmd.password))
			return 1;
        if (!recv_line_timeout(b, ln, 5000) || ln.rfind("+OK", 0) != 0)
		{
            ...
            size_t p = ln.find("uid=");
            if (p != std::string::npos) {
                try { cps[i].uidB = std::stoll(ln.substr(p+4)); } catch (...) { cps[i].uidB = 0; }
            }
        }
        cps[i].a = a; cps[i].b = b;
	}
	...
	auto worker = [&](int tid)
	{
		...
		for (;;)
		{
			...
			for (int i = beg; i < end; i++)
			{
				...
				// 使用登录返回的真实 uidB 作为目标
				if (cps[i].uidB == 0) continue;
				std::string to = std::to_string(cps[i].uidB);
				if (!send_line(cps[i].a, std::string("SEND ") + to + " " + payload))
					continue;
				std::string ln;
                if (!recv_line_timeout(cps[i].a, ln, 5000))
					continue;
