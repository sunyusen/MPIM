...
struct Cmd {
    ...
    int port = 7000; // im-gateway 监听端口（请按实际网关端口传参）
    int pairs = 4;   // 会话对数量（每对2个连接：sender/receiver）
    int threads = 4; // 工作线程数
    int seconds = 15;
    int payload = 64;
    std::string user_base = "user"; // 账号前缀：user1,user2,...
...
static bool recv_line(int fd, std::string& out) {
    ...
    while (true) {
        ...
        if (n > 0) {
            ...
            if (out.size() > 65536) return false; // 防御
            continue;
        }
...
static int dial(const std::string& host, int port) {
    ...
    // 读欢迎行
    std::string line; recv_line(fd, line);
    return fd;
}

struct Stats { std::atomic<uint64_t> ok{0}, fail{0}; std::vector<double> lat_ms; std::mutex mu; };

static double pct(std::vector<double>& v, double p) {
    if (v.empty()) return 0; size_t k = (size_t)std::llround((p/100.0)*(v.size()-1));
...
int main(int argc, char** argv) {
    ...
    for (int i=1;i<argc;i++){
        std::string a=argv[i];
        auto eat=[&](const char* key, auto& dst){ if(a.rfind(key,0)==0){ dst=decltype(dst)(std::stoi(a.substr(strlen(key)))); return true;} return false;};
        if (a.rfind("--host=",0)==0) cmd.host = a.substr(7);
        else if (a.rfind("--port=",0)==0) eat("--port=", cmd.port);
        else if (a.rfind("--pairs=",0)==0) eat("--pairs=", cmd.pairs);
        else if (a.rfind("--threads=",0)==0) eat("--threads=", cmd.threads);
    ...
    struct ConnPair { int a{-1}, b{-1}; };
    std::vector<ConnPair> cps(cmd.pairs);
    for (int i=0;i<cmd.pairs;i++){
        int a = dial(cmd.host, cmd.port); if(a==-1){ std::cerr<<"dial A fail\n"; return 1; }
        int b = dial(cmd.host, cmd.port); if(b==-1){ std::cerr<<"dial B fail\n"; return 1; }
        ...
        if(!send_line(a, "LOGIN "+userA+" "+cmd.password)) return 1; std::string ln; if(!recv_line(a,ln) || ln.rfind("+OK",0)!=0){ std::cerr<<"login A fail: "<<ln<<"\n"; return 1; }
        if(!send_line(b, "LOGIN "+userB+" "+cmd.password)) return 1; if(!recv_line(b,ln) || ln.rfind("+OK",0)!=0){ std::cerr<<"login B fail: "<<ln<<"\n"; return 1; }
        cps[i]={a,b};
    }

    std::atomic<bool> stop{false}; Stats st_send, st_pull;
    ...
    auto worker = [&](int tid){
        ...
        for(;;){ if(stop.load()) break; for(int i=beg;i<end;i++){
            // A -> B 发送
            auto t0 = steady_clock::now();
            // 目标 uid 这里假设为 (2*i+2)，与登录策略一致
            std::string to = std::to_string(2*i+2);
            if(!send_line(cps[i].a, std::string("SEND ")+to+" "+payload)) continue;
            std::string ln; if(!recv_line(cps[i].a, ln)) continue;
    ...
    for(int t=0;t<cmd.threads;t++) th.emplace_back(worker, t);
    std::this_thread::sleep_for(std::chrono::seconds(cmd.seconds));
    stop.store(true);
    for(auto& t: th) t.join();

    // 关闭连接
    for (auto &cp: cps){ if(cp.a!=-1) ::close(cp.a); if(cp.b!=-1) ::close(cp.b);}    
