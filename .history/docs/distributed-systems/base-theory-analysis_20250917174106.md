...
**实现细节**：
```cpp
// 在 im-common/include/version_control.h 中
...
public:
    ...
    // 更新版本号
    bool UpdateVersion(const std::string& key, int64_t version);
    
    // 检查版本冲突
    bool CheckConflict(const std::string& key, int64_t expected_version);
    
    // 解决版本冲突
    bool ResolveConflict(const std::string& key, int64_t new_version);
