...
**实现细节**：
```cpp
// 在 mprpc/src/two_phase_commit.h 中
...
public:
    ...
    // 开始事务
    bool BeginTransaction(const std::vector<std::string>& participants);
...
private:
    ...
    std::unordered_map<std::string, bool> prepare_results_;
    bool is_prepared_;
    bool is_committed_;
    
    // 发送准备请求
    bool SendPrepareRequest(const std::string& participant);
    
    // 发送提交请求
    bool SendCommitRequest(const std::string& participant);
    
    // 发送回滚请求
    bool SendRollbackRequest(const std::string& participant);
};
```
...
**实现细节**：
```cpp
// 在 mprpc/src/three_phase_commit.h 中
...
private:
    std::vector<std::string> participants_;
    std::unordered_map<std::string, bool> prepare_results_;
    std::unordered_map<std::string, bool> precommit_results_;
    bool is_prepared_;
    bool is_precommitted_;
    bool is_committed_;
    ...
    // 发送预提交请求
    bool SendPreCommitRequest(const std::string& participant);
