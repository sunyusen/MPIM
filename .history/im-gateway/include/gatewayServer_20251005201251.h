...
private:
	...
	// 处理从 Redis 接收到的消息
	void handleRedisMessage(const std::string& payload);
...
private:
    ...
    std::unique_ptr<MprpcChannel> ch_group_;
    ...
    std::unique_ptr<mpim::GroupService_Stub>    group_;

	// Redis消息队列： 订阅本网关通道，接收 Presence.Deliver 投递的在线消息
