	...
	: server_(loop, addr, "im-gateway"), gateway_id_(gateway_id)
{
	...
	if (redis_.connect())
	{
		std::cout << "Gateway: Redis connected successfully" << std::endl;
		int ch = gatewayChannel();
		std::cout << "Gateway: Subscribing to channel " << ch << std::endl;
		redis_.init_notify_handler([this](int /*channel*/, std::string payload)
								   {
            std::cout << "Gateway: Received message from Redis, payload size=" << payload.size() << std::endl;
            mpim::C2CMsg m;
            if(!m.ParseFromString(payload)) {
                LOG_WARN << "bad payload for C2CMsg";
                return;
            }
            std::cout << "Gateway: Parsed message to=" << m.to() << " from=" << m.from() << " text=" << m.text() << std::endl;
            auto it = uid2conn_.find(m.to());
            if (it != uid2conn_.end()) {
                if(auto conn = it->second.lock()){
                    std::cout << "Gateway: Found connection for user " << m.to() << ", sending message" << std::endl;
                    ...
                    std::cout << "Gateway: Connection for user " << m.to() << " is expired" << std::endl;
                }
            } else {
                std::cout << "Gateway: No connection found for user " << m.to() << std::endl;
            } });
		redis_.subscribe(ch);
	}
	else
...
bool GatewayServer::handleLOGIN(const TcpConnectionPtr &c, const std::vector<std::string> &toks)
{
	...
	if (ctl2.Failed()){
		...
		LOG_WARN << "Presence.BindRoute returned !OK: code=" << brs.result().code()
		      << " msg=" << brs.result().msg();
	}
