	...
	: server_(loop, addr, "im-gateway"), gateway_id_(gateway_id)
{
	...
	if (redis_.connect())
	{
		...
		LOG_INFO("Gateway: Subscribing to channel " + std::to_string(ch));
		redis_.init_notify_handler([this](int /*channel*/, std::string payload)
								   {
            LOG_DEBUG("Gateway: Received message from Redis, payload size=" + std::to_string(payload.size()));
            mpim::C2CMsg m;
            if(!m.ParseFromString(payload)) {
                LOG_WARN("bad payload for C2CMsg");
                return;
            }
            LOG_DEBUG("Gateway: Parsed message to=" + std::to_string(m.to()) + " from=" + std::to_string(m.from()) + " text=" + m.text());
            auto it = uid2conn_.find(m.to());
            if (it != uid2conn_.end()) {
                if(auto conn = it->second.lock()){
                    LOG_DEBUG("Gateway: Found connection for user " + std::to_string(m.to()) + ", sending message");
                    ...
                    LOG_WARN("Gateway: Connection for user " + std::to_string(m.to()) + " is expired");
                }
            } else {
                LOG_WARN("Gateway: No connection found for user " + std::to_string(m.to()));
            } });
		redis_.subscribe(ch);
	}
	else
...
void GatewayServer::onConnection(const TcpConnectionPtr &c)
{
	...
	else
	{
		...
		if (it != sessions_.end() && it->second.authed)
		{
			...
			if (ctl.Failed()) {
				LOG_ERROR("Failed to update user state to offline for uid=" + std::to_string(uid));
			} else {
				LOG_INFO("User " + std::to_string(uid) + " state updated to offline due to connection close");
			}
		}
...
bool GatewayServer::handleLOGIN(const TcpConnectionPtr &c, const std::vector<std::string> &toks)
{
	...
	if (ctl2.Failed()){
		...
		LOG_WARN("Presence.BindRoute returned !OK: code=" + std::to_string(brs.result().code()) + " msg=" + brs.result().msg());
	}
