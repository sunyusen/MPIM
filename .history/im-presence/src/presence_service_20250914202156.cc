...
PresenceServiceImpl::PresenceServiceImpl()
{
	...
	while (retry_count < max_retries) {
		...
		retry_count++;
		LOG_WARN("Redis connection attempt " + std::to_string(retry_count) + " failed, retrying...");
		std::this_thread::sleep_for(std::chrono::seconds(1));
	}
	
	LOG_ERROR("Redis connection failed after " + std::to_string(max_retries) + " attempts! Service will start but message delivery will be disabled.");
									...
									google::protobuf::Closure *done)
{
	...
	LOG_DEBUG("BindRoute: user_id=" + std::to_string(req->user_id()) + 
	...
	if (!redis_connected_) {
		...
		if (r_.connect()) {
			...
			LOG_ERROR("Redis reconnection failed, cannot bind route for user " + std::to_string(req->user_id()));
	...
	try {
		...
		{
			LOG_INFO("Route bound successfully for user " + std::to_string(req->user_id()));
