									...
									google::protobuf::Closure *done)
{
	...
	if (!redis_connected_) {
		std::cout << "Redis not connected, cannot bind route for user " << req->user_id() << std::endl;
									 ...
									 google::protobuf::Closure *done)
{
	...
	// Check if Redis is connected
	if (!redis_connected_) {
		std::cout << "Redis not connected, cannot query route for user " << req->user_id() << std::endl;
		resp->mutable_result()->set_code(Code::INTERNAL);
		resp->mutable_result()->set_msg("redis not connected");
		if (done) done->Run();
		return;
