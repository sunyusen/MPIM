...
void Client::stop()
{
	...
	// 如果用户已登录，发送登出命令
	if (loginSuccess.load() && clientfd != -1) {
		std::cout << "Sending logout command before disconnect..." << std::endl;
		enqueueRaw("LOGOUT");
		// 给一点时间让登出命令发送
...
void Client::handleLogoutResponse()
{
	loginSuccess.store(false);
	uid_.store(0);
	std::cout << "\r✅ logout success，已退出登录。\n";
}
...
void Client::handleChatResponse(const std::string &response)
{
	...
	if (response.find("MSG ") == 0) {
		...
		std::cout << "[uid=" << (loginSuccess.load() ? std::to_string(uid_.load()) : "?") << "] > ";
		std::cout.flush();
	} else {
...
void Client::prettyPrintLine(const std::string &line)
{
	...
	else if (line.rfind("+OK logout", 0) == 0)
	{
