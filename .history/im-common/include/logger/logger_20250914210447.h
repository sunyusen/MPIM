...
// 便捷的日志宏定义 - 支持 << 操作符形式
#define LOG_TRACE if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::TRACE)) \
    mpim::logger::LogStream(mpim::logger::TRACE, __FILE__, __FUNCTION__, __LINE__)

#define LOG_DEBUG if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::DEBUG)) \
    mpim::logger::LogStream(mpim::logger::DEBUG, __FILE__, __FUNCTION__, __LINE__)

#define LOG_INFO if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::INFO)) \
    mpim::logger::LogStream(mpim::logger::INFO, __FILE__, __FUNCTION__, __LINE__)

#define LOG_WARN if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::WARN)) \
    mpim::logger::LogStream(mpim::logger::WARN, __FILE__, __FUNCTION__, __LINE__)

#define LOG_ERROR if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::ERROR)) \
    mpim::logger::LogStream(mpim::logger::ERROR, __FILE__, __FUNCTION__, __LINE__)

#define LOG_FATAL if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::FATAL)) \
    mpim::logger::LogStream(mpim::logger::FATAL, __FILE__, __FUNCTION__, __LINE__)

// 支持字符串参数的日志宏（兼容旧语法）
#define LOG_TRACE_STR(msg) do { \
    if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::TRACE)) { \
        mpim::logger::Logger::GetInstance().Log(mpim::logger::TRACE, msg, __FILE__, __FUNCTION__, __LINE__); \
    } \
} while(0)

#define LOG_DEBUG_STR(msg) do { \
    if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::DEBUG)) { \
        mpim::logger::Logger::GetInstance().Log(mpim::logger::DEBUG, msg, __FILE__, __FUNCTION__, __LINE__); \
    } \
} while(0)

#define LOG_INFO_STR(msg) do { \
    if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::INFO)) { \
        mpim::logger::Logger::GetInstance().Log(mpim::logger::INFO, msg, __FILE__, __FUNCTION__, __LINE__); \
    } \
} while(0)

#define LOG_WARN_STR(msg) do { \
    if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::WARN)) { \
        mpim::logger::Logger::GetInstance().Log(mpim::logger::WARN, msg, __FILE__, __FUNCTION__, __LINE__); \
    } \
} while(0)

#define LOG_ERROR_STR(msg) do { \
    if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::ERROR)) { \
        mpim::logger::Logger::GetInstance().Log(mpim::logger::ERROR, msg, __FILE__, __FUNCTION__, __LINE__); \
    } \
} while(0)

#define LOG_FATAL_STR(msg) do { \
    if (mpim::logger::Logger::GetInstance().IsEnabled(mpim::logger::FATAL)) { \
...
#define LOG_TRACE(msg) LOG_TRACE_STR(msg)
#define LOG_DEBUG(msg) LOG_DEBUG_STR(msg)
#define LOG_INFO(msg) LOG_INFO_STR(msg)
#define LOG_WARN(msg) LOG_WARN_STR(msg)
#define LOG_ERROR(msg) LOG_ERROR_STR(msg)
#define LOG_FATAL(msg) LOG_FATAL_STR(msg)
