...
void LogMacro(LogLevel level, const std::string& file, const std::string& func, int line, Args&&... args) {
    if (Logger::GetInstance().IsEnabled(level)) {
        if constexpr (sizeof...(args) == 1) {
            // 单个参数，直接调用Log
            Logger::GetInstance().Log(level, std::forward<Args>(args)..., file, func, line);
        } else {
            // 多个参数，使用LogStream
            LogStream stream(level, file, func, line);
            (stream << ... << args);
        }
    }
}

// 日志宏定义 - 同时支持 << 操作符和字符串参数两种形式
#define LOG_TRACE(...) mpim::logger::LogMacro(mpim::logger::TRACE, __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
#define LOG_DEBUG(...) mpim::logger::LogMacro(mpim::logger::DEBUG, __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
#define LOG_INFO(...) mpim::logger::LogMacro(mpim::logger::INFO, __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
#define LOG_WARN(...) mpim::logger::LogMacro(mpim::logger::WARN, __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
#define LOG_ERROR(...) mpim::logger::LogMacro(mpim::logger::ERROR, __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
#define LOG_FATAL(...) mpim::logger::LogMacro(mpim::logger::FATAL, __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
