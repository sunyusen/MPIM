...
private:
    ...
    void testGroupCache() {
        ...
        // 测试更新群成员列表
        std::cout << "4. Testing update group members..." << std::endl;
        std::string newMembersData = "123,456,789,999";
        bool updateMembers = groupCache.SetGroupMembers(groupId, newMembersData, 60);
        std::cout << "   Update group members: " << (updateMembers ? "SUCCESS" : "FAILED") << std::endl;
        
        std::string updatedMembers = groupCache.GetGroupMembers(groupId);
        std::cout << "   Get updated members: " << (updatedMembers == newMembersData ? "SUCCESS" : "FAILED") << std::endl;
        std::cout << "   Updated members: " << updatedMembers << std::endl;
