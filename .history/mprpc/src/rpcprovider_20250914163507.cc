...
// 支持智能指针的NotifyService重载
template<typename T>
void RpcProvider::NotifyService(std::unique_ptr<T> service) {
	ServiceInfo service_info;
	...
	service_info.m_service = service.get();
	service_info.m_service_owner = std::move(service); // 转移所有权
	std::cout << "service_name: " << service_name << " method_count: " << methodCnt << std::endl;
	m_serviceMap.insert({service_name, service_info});
}
							...
							muduo::net::Buffer *buffer, muduo::Timestamp)
{
	...
	while (true)
	{
		...
		if (!request)
		{
			std::cout << "Failed to create request message" << std::endl;
			continue;
		}
