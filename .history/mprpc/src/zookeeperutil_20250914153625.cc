...
void ZkClient::Start()
{
	...
	*/
	...
	if (state == ZOO_CONNECTED_STATE) {
		...
		std::cout << "ZooKeeper connection state: " << state << std::endl;
	}
}
...
void ZkClient::Create(const char *path, const char *data, int datalen, int state)
{
	...
	if (ZNONODE == flag)	//表示path的znode节点不存在
	{
		...
		{
			std::cout << "znode create error... path: " << path << " flag: " << flag << std::endl;
			// 不要直接退出，而是记录错误并继续
			// 某些节点可能已经存在，这是正常的
			if (flag == ZNODEEXISTS) {
				std::cout << "znode already exists, continuing..." << std::endl;
			} else {
				std::cout << "znode create failed with error code: " << flag << std::endl;
			}
		}
	}
}
