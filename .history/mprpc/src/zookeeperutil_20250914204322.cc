...
void ZkClient::Start()
{
	...
	*/
	...
	if (state == ZOO_CONNECTED_STATE) {
		...
		LOG_WARN("ZooKeeper connection state: " + std::to_string(state));
	}
}
...
void ZkClient::Create(const char *path, const char *data, int datalen, int state)
{
	...
	if (ZNONODE == flag)	//表示path的znode节点不存在
	{
		...
		if (flag == ZOK)
		{
			LOG_DEBUG("znode create success... path: " + std::string(path));
		}
		else
		{
			LOG_ERROR("znode create error... path: " + std::string(path) + " flag: " + std::to_string(flag));
			// 不要直接退出，而是记录错误并继续
			// 某些节点可能已经存在，这是正常的
			if (flag == ZNODEEXISTS) {
				LOG_DEBUG("znode already exists, continuing...");
			} else {
				LOG_ERROR("znode create failed with error code: " + std::to_string(flag));
			}
		}
	}
}
...
std::string ZkClient::GetData(const char *path)
{
	...
	if (flag != ZOK)
	{
		LOG_ERROR("get znode error... path: " + std::string(path));
		return "";
	}
	else
	{
