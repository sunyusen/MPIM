...
MessageServiceImpl::MessageServiceImpl()
{
	...
	try {
		...
		LOG_ERROR("MessageServiceImpl: Exception during initialization: " + std::string(e.what()));
		ch_presence_.reset();
		presence_.reset();
	}
}
							  ...
							  google::protobuf::Closure *done)
{
	...
	if (!ch_presence_ || !presence_) {
		std::cout << "MessageService::Send: Presence RPC channel or stub is null!" << std::endl;
