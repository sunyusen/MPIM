...
OfflineModel::OfflineModel()
{
	db_.reset(new MySQL());
	if (!db_->connect()) {
		std::cout << "OfflineModel: MySQL connection failed! Offline message storage will be disabled." << std::endl;
...
{
	if (!db_) {
		std::cout << "OfflineModel::insert: Database not available, skipping offline storage" << std::endl;
		return false;
	}
...
{
	if (!db_) {
		std::cout << "OfflineModel::query: Database not available, returning empty result" << std::endl;
		return {};
	}
...
{
	if (!db_) {
		std::cout << "OfflineModel::remove: Database not available, skipping remove operation" << std::endl;
		return false;
	}
