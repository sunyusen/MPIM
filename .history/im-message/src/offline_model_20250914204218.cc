...
bool OfflineModel::insert(int64_t uid, const std::string &payload)
{
	...
	LOG_DEBUG("OfflineModel::insert: uid=" + std::to_string(uid) + " payload_size=" + std::to_string(payload.size()) + " base64_size=" + std::to_string(base64_payload.size()));
	
	bool result = db_->update(os.str());
	LOG_DEBUG("OfflineModel::insert: result=" + std::string(result ? "success" : "failed"));
	return result;
}
